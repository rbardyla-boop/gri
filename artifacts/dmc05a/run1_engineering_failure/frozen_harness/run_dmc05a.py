...
def conventional_case(system: str, case: dict[str, Any]) -> dict[str, Any]:
    ...
    if system == "full_history_scan":
        ...
        for row in stream:
            ...
            index[f"{entry['key'][0]}:{entry['key'][1]}"].append(entry)
        state = dict(sorted(index.items()))
        persistent_payload = state
        discarded = 0
    elif system == "conventional_retrieval":
        state = [compact_entry(row) for row in stream]
        persistent_payload = state
        discarded = 0
    else:
    ...
    if system in {"full_history_scan", "recent_window_16", "frozen_fifo_16", "random_16"}:
        candidates = [compact_entry(row) for row in state]
        selected = exact_select(candidates, qkey, mode, as_of)
        query_inspected = len(candidates)
        retrieval_operations = len(candidates)
        working_payload: Any = state
        critical_ids = {str(row["record_id"]) for row in state}
    elif system == "exact_structured":
        bucket = state.get(f"{qkey[0]}:{qkey[1]}", [])
        ...
        critical_ids = {str(row["record_id"]) for bucket_rows in state.values() for row in bucket_rows}
    else:
        scored = []
        for row in state:
            key = tuple(row["key"])
        ...
        selected = exact_select(candidates, qkey, mode, as_of)
        query_inspected = len(state)
        retrieval_operations = len(state)
        working_payload = candidates
...
def dmc_case(system: str, case: dict[str, Any], models: dict[str, Any], module: Any) -> dict[str, Any]:
    ...
    return {
        ...
        "retrieval_model_forward_calls": 1,
...
def calculate_gates(aggregate: dict[str, Any], integrity: dict[str, bool]) -> dict[str, Any]:
    ...
    conventional_bounded = max(
        metric(aggregate, "exact_structured", size, "maximum_working_set_records")
        for size in HISTORY_SIZES
    ) <= CAPACITY and max(
        metric(aggregate, "conventional_retrieval", size, "maximum_working_set_records")
        for size in HISTORY_SIZES
    ) <= CAPACITY
    gates = {
    ...
    conventional_pareto = (
        structured_accuracy >= dmc_accuracy - 0.01
        and structured_bytes_1024 <= dmc_bytes_1024
        and metric(aggregate, "exact_structured", 1024, "working_set_serialized_bytes") <= metric(aggregate, "dmc04b", 1024, "working_set_serialized_bytes")
        and metric(aggregate, "exact_structured", 1024, "records_inspected_query") <= metric(aggregate, "dmc04b", 1024, "records_inspected_query")
        and metric(aggregate, "exact_structured", 1024, "query_wall_ns") <= metric(aggregate, "dmc04b", 1024, "query_wall_ns")
    )
    accounting_valid = all(integrity.values())
    bounded_advantage = gates["dmc_capability"]["pass"] and (
        not gates["exact_structured_capability"]["pass"]
        or not gates["conventional_retrieval_capability"]["pass"]
        or not gates["conventional_bounded_working_set"]["pass"]
    )
    storage_only = (
...
def write_curves(aggregate: dict[str, Any]) -> dict[str, Any]:
    ...
    curves = {
        label: {
            system: {str(size): metric(aggregate, system, size, name) for size in HISTORY_SIZES}
            for system in systems
        }
        for label, name in curve_metrics.items()
    }
    write_json(OUT / "scaling_curves.json", curves)
    csv_lines = ["history_size,system,answer_accuracy,persistent_bytes,query_records_inspected,active_working_set_records,query_wall_ns"]
    for size in HISTORY_SIZES:
        for system in systems:
            csv_lines.append(
                ",".join(
    ...
    labels = [
        ...
        ("query_wall_ns", "Query wall time (ns)", True),
    ]
    for axis, (name, title, log_scale) in zip(axes, labels):
        for system in systems:
            axis.plot(HISTORY_SIZES, [metric(aggregate, system, size, name) for size in HISTORY_SIZES], marker="o", label=system)
...
def report_markdown(terminal: str, aggregate: dict[str, Any], gates: dict[str, Any], training: dict[str, Any]) -> str:
    ...
    for system in ALL_SYSTEMS:
        ...
        lines.append(
            ...
            f"{metric(aggregate, system, 1024, 'query_wall_ns'):.3f} | {forwards:.3f} |"
        )
    lines.extend([
        "",
        "## Gates",
...
def verify_freeze() -> dict[str, Any]:
    ...
    if freeze.get("status") != "FROZEN_BEFORE_SCIENTIFIC_RUN":
...
def run_one_worker(system: str, seed: int | None, output: Path) -> dict[str, Any]:
    ...
    payload["raw_receipt"] = str(output.relative_to(ROOT))

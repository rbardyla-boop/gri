...
from authorize_sem0r3 import SUCCESSOR_SOURCE_FILES, verify_qualification  # noqa: E402
...
For every proposition, choose exactly one label from: ASSERTED, ENTAILED, PRESUPPOSED, IMPLICATED, CONTRADICTED, UNKNOWN.
Return only JSON matching the supplied schema. The predictions object is keyed by proposition ID. Each proposition has exactly label and evidence. Evidence is an object keyed only by supplied context statement IDs; set a statement ID to true iff it directly supports the classification. You may omit non-evidence IDs or set them false.
...
def request_raw(*, root: str, model: str, visible_case: dict[str, Any], schema: dict[str, Any], seed: int) -> bytes:
    ...
    req = urllib.request.Request(
        ...
        headers={'Content-Type': 'application/json', 'User-Agent': 'gri-sem0r3/1'},
        method='POST',
    )
...
def parse_inner(raw: bytes) -> dict[str, Any]:
    ...
    parsed = loads_no_duplicates(content)
...
def verify_authorization(*, auth_path: Path, manifest_path: Path, identity_path: Path, qualification_path: Path,
    ...
    if auth.get('status') != 'SEM0R3_ONE_RUN_AUTHORIZED' or auth.get('executions_authorized') != 1 or auth.get('consumed') is not False:
        raise ValueError('SEM-0R3 authorization invalid or already consumed')
    ...
    if observed != parent_digest(body):
        raise ValueError('SEM-0R3 authorization digest mismatch')
...
def consume_authorization(path: Path, auth: dict[str, Any], receipt_path: Path) -> None:
    ...
    consumed['status'] = 'SEM0R3_ONE_RUN_CONSUMED'
    ...
    receipt = {
        'unit': 'SEM-0R3',
        'status': 'SEM0R3_EXECUTION_STARTED',
...
def run_phase(*, phase: str, cases: list[dict[str, Any]], predictions_path: Path, raw_log_path: Path,
    ...
    with predictions_path.open('x', encoding='utf-8') as pred_handle, raw_log_path.open('x', encoding='utf-8') as raw_handle:
        for index, case in enumerate(cases):
            ...
            raw = request_raw(root=root, model=model, visible_case=model_view(case), schema=wire_schema(case, LABELS), seed=case_seed)
            append_raw(raw_handle, phase=phase, ordinal=index, case_id=case['id'], raw=raw)
            wire_payload = parse_inner(raw)
            parent_payload = wire_to_parent(case, wire_payload)
            errors = validate_prediction_payload(case, parent_payload)
            if errors:
                raise ValueError(f'{phase} malformed translated model output at ordinal {index}: {errors}')
            pred_handle.write(json.dumps({'case_id': case['id'], 'payload': parent_payload}, sort_keys=True) + '\n')
...
def main() -> None:
    ...
    try:
        ...
        receipt.update({'status': 'SEM0R3_EXECUTION_TERMINATED', 'ended_at': datetime.now(timezone.utc).isoformat(), 'error_type': type(exc).__name__, 'error': str(exc)})
        paths['receipt'].write_text(json.dumps(receipt, indent=2, sort_keys=True) + '\n', encoding='utf-8')
        raise

    receipt = load_json(paths['receipt'])
    receipt.update({'status': 'SEM0R3_PREDICTIONS_ALL_SEALED', 'ended_at': datetime.now(timezone.utc).isoformat()})
    paths['receipt'].write_text(json.dumps(receipt, indent=2, sort_keys=True) + '\n', encoding='utf-8')

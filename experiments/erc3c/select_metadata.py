...
def download(url: str) -> bytes:
    request = urllib.request.Request(url, headers={"User-Agent": "gri-erc3c/1.0"})
...
def load_exclusion_union(
    *,
    old_map: Path = OLD_ERC3A_MAP,
    erc3b_calibration_map: Path = ERC3B_CALIBRATION_MAP,
    erc3b_science_map: Path = ERC3B_SCIENCE_MAP,
) -> tuple[set[int], dict[str, str]]:
    old = _map_ids(old_map, 64, EXPECTED_ERC3A_IDS_SHA256)
    calibration = _map_ids(erc3b_calibration_map, 8, EXPECTED_ERC3B_CALIBRATION_IDS_SHA256)
    science = _map_ids(erc3b_science_map, 64, EXPECTED_ERC3B_SCIENCE_IDS_SHA256)
    if old & calibration or old & science or calibration & science:
        raise ValueError("predecessor exclusion sets overlap")
    union = old | calibration | science
    return union, {
        "erc3a_ids_sha256": sha256_json(sorted(old)),
        "erc3b_calibration_ids_sha256": sha256_json(sorted(calibration)),
        "erc3b_science_ids_sha256": sha256_json(sorted(science)),
        "exclusion_union_sha256": sha256_json(sorted(union)),
    }
...
def select(rows: list[dict], exclusions: set[int]) -> tuple[list[dict], list[dict], list[dict], list[dict], list[dict]]:
    ...
    if calibration_ids & science_ids or science_ids & exclusions:
        raise ValueError("ERC-3C selection overlaps permanent exclusions or calibration")
    ...
    for row in sorted(calibration_rows, key=lambda item: item["sample_id"]):
        opaque = opaque_id("P90C-CAL", row["sample_id"])
    ...
    for row in sorted(science_rows, key=lambda item: item["sample_id"]):
        opaque = opaque_id("P90C-SCI", row["sample_id"])
...
def main() -> None:
    ...
    _write(args.out_dir / "ERC3C_CALIBRATION_ACQUISITION_MAP.json", calibration)
    _write(args.out_dir / "ERC3C_CALIBRATION_PUBLIC_SELECTION.json", calibration_public)
    _write(args.out_dir / "ERC3C_SCIENCE_ACQUISITION_MAP.json", science)
    _write(args.out_dir / "ERC3C_PUBLIC_SELECTION.json", science_public)
    _write(args.out_dir / "ERC3C_SCORER_MAP.json", scorer)
    calibration_ids = [row["sample_id"] for row in calibration]
    science_ids = [row["sample_id"] for row in science]
    record = {
        "unit": "ERC-3C",
        "status": "ERC3C_METADATA_SELECTION_FROZEN",
    ...
    _write(args.out_dir / "ERC3C_METADATA_SELECTION_RECORD.json", record)

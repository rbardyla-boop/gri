...
FORBIDDEN_PRODUCER_FIELDS = frozenset(
    {
        "sample_id", "fault_target", "sc_type", "sc_location",
        "fault_resistance", "resistance", "truth", "scorer_map",
        "fault_phase", "phase_label",
    }
)
...
def assert_clean(paths: Iterable[Path]) -> None:
    ...
    if violations:
        raise AssertionError(f"ERC3D producer identity leakage: {json.dumps(violations, sort_keys=True)}")
...
def main() -> None:
    ...
    print("ERC3D_PRODUCER_IDENTITY_BOUNDARY_PASS")

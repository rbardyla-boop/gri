...
from experiments.erc3b.remote_zip_index import central_directory_location, fetch_range, head_size, parse_central_directory
...
def build_index(calibration_map: Path, science_map: Path, output: Path) -> dict:
    selected_maps = _load_map(calibration_map, "calibration", 8) + _load_map(science_map, "science", 64)
    if len({row["opaque_id"] for row in selected_maps}) != 72 or len({int(row["sample_id"]) for row in selected_maps}) != 72:
        raise ValueError("ERC-3D selected IDs are not unique")
    ...
    record = {
        "unit": "ERC-3D",
        "status": "ERC3D_REMOTE_ZIP_INDEX_CAPTURED",

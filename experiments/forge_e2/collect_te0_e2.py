...
from experiments.forge_e1.collect_te0_e1 import SYSTEM_PROMPT

EXPECTED_STATUS = "TE0_E2_MODEL_PREFLIGHT_PASS"
...
def seed_for(case_id: str) -> int:
    return int.from_bytes(hashlib.sha256(("TE0-E2:" + case_id).encode("utf-8")).digest()[:4], "big") & 0x7FFFFFFF


def load_identity(path: Path) -> dict[str, Any]:
    value = json.loads(path.read_text(encoding="utf-8"))
    if not isinstance(value, dict) or value.get("status") != EXPECTED_STATUS:
        raise ValueError("TE0_E2_MODEL_IDENTITY_NOT_QUALIFIED")
    observed = value.get("record_sha256")
    body = {k: v for k, v in value.items() if k != "record_sha256"}
    if observed != hashlib.sha256(canonical(body)).hexdigest():
        raise ValueError("TE0_E2_MODEL_IDENTITY_DIGEST_MISMATCH")
    return value
...
def load_pool(path: Path) -> list[dict[str, Any]]:
    ...
    if not rows:
        raise ValueError("TE0_E2_EMPTY_POOL")
    seen: set[str] = set()
    for row in rows:
        if not isinstance(row, dict) or set(row) != {"case_id", "prompt", "target", "template_index"}:
            raise ValueError("TE0_E2_POOL_ROW_SHAPE_INVALID")
        cid = str(row["case_id"])
        if cid in seen:
            raise ValueError(f"TE0_E2_DUPLICATE_CASE:{cid}")
        seen.add(cid)
    return rows


def main() -> None:
    ap = argparse.ArgumentParser(description="Collect TE0-E2 raw producer outputs for BUILD or DEV only. No Vault argument exists.")
    ...
    if args.output.exists() or args.receipt.exists():
        raise FileExistsError("TE0_E2_REFUSE_OVERWRITE")
    ...
    for ordinal, row in enumerate(rows):
        ...
        if not isinstance(content, str):
            raise RuntimeError(f"TE0_E2_MISSING_CONTENT:{cid}")
        raw_records.append({"case_id": cid, "input": content, "expected": row["target"]})
        trace_rows.append({
            "ordinal": ordinal,
            "case_id": cid,
    ...
    receipt = {
        "schema_version": 1,
        "unit": "TE0-E2",
        "status": "TE0_E2_COLLECTION_COMPLETE",
        "phase": args.phase,
        "scientific": False,

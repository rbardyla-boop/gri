...
def verify_qualification(path: Path, identity_path: Path) -> dict[str, Any]:
    ...
    if value.get('status') != 'SEM0R2_INTERFACE_QUALIFICATION_PASS':
...
def create_authorization(*, manifest_path: Path, identity_path: Path, qualification_path: Path,
    ...
    record: dict[str, Any] = {
        'schema_version': 1,
        'unit': 'SEM-0R2',
        'status': 'SEM0R2_ONE_RUN_AUTHORIZED',
        'created_at': datetime.now(timezone.utc).isoformat(),
        'executions_authorized': 1,
        'consumed': False,
        'scientific_scope': 'one complete SEM-0R2 interface-repaired replication using unchanged SEM-0R semantic content',
        'bindings': {
        ...
        'prohibitions': {
            'run001_continuation_claim': True,

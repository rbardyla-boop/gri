"""Fail-closed Dual-Authority-0.3 scientific runner.
...
Nursery, while the epistemic challenge and controls use the 0.3 interfaces.
"""
...
from . import alternate_evidence, design, store as d
...
AUTHORIZED_DEVELOPMENT_SEED = 340
...
def _build_alternate_challenge(
    episode_ordinal: int,
    *,
    start_tick: int,
    phase: PhaseRecorder | None = None,
    ...
    with _phase_context(
        phase,
        "alternate_evidence_challenge",
        design.ALTERNATE_VALID_CASES_PER_EPISODE,
    ):
        return alternate_evidence.build_alternate_evidence_challenge(
            episode_ordinal,
            valid_cases=design.ALTERNATE_VALID_CASES_PER_EPISODE,
            include_hostile=True,
            start_tick=start_tick,
        )


def _epistemic_gates(
    transition_metrics: dict[str, object],
    alternate: dict[str, object],
) -> tuple[dict[str, bool], str]:
    """Return scientific epistemic gates without turning a scientific fail
    into an operational artifact-validation exception.
    """

    metric_a = transition_metrics["alternate_support_preservation"]
    metric_b = transition_metrics["recomputed_after_parent_change"]
    gates = {
        "alternate_support_opportunities_min": (
            int(metric_a["opportunities"])
            >= design.ALTERNATE_SUPPORT_MIN_OPPORTUNITIES
        ),
        "alternate_support_preservation_rate_exact": (
            float(metric_a["rate"]) == 1.0
        ),
        "false_opportunity_classifications_zero": (
            int(alternate["false_opportunity_classifications"]) == 0
        ),
        "recomputation_opportunities_min": (
            int(metric_b["opportunities"]) >= design.RECOMPUTATION_MIN_OPPORTUNITIES
        ),
        "recomputation_precision_exact": (
            float(metric_b["global_precision"]) == 1.0
        ),
        "recomputation_recall_exact": float(metric_b["global_recall"]) == 1.0,
    }
...
def run_engineering_profile(
    ...
    profile_alternate_events: list[dict[str, object]] = []
    profile_alternate_summaries: list[dict[str, object]] = []
    ...
    if "pipeline" in phases:
        ...
        for ordinal, mode in enumerate(MODES):
            ...
            alternate_frames, alternate_events, alternate_summary = _build_alternate_challenge(
                ordinal,
                start_tick=1_000_000 + ordinal * 100_000,
                phase=phase,
            )
            challenge_frames.extend(alternate_frames)
            profile_alternate_events.extend(alternate_events)
            profile_alternate_summaries.append(alternate_summary)
        profile["pipeline"] = {
            ...
            "alternate_evidence_events": len(profile_alternate_events),
        }
    ...
    if profile_alternate_events:
        profile["alternate_evidence"] = {
            "guaranteed_valid_cases_per_episode": design.ALTERNATE_VALID_CASES_PER_EPISODE,
            "guaranteed_opportunities": design.ALTERNATE_VALID_CASES_PER_EPISODE
            * len(profile_alternate_summaries),
            "guaranteed_positive_event_count": design.ALTERNATE_VALID_CASES_PER_EPISODE
            * len(profile_alternate_summaries),
            "additional_hostile_event_count": (
                len(alternate_evidence.EMITTED_HOSTILE_CASE_CODES)
                * len(profile_alternate_summaries)
            ),
            "represented_hostile_case_codes": list(alternate_evidence.HOSTILE_CASE_CODES),
            "emitted_hostile_case_codes": list(
                alternate_evidence.EMITTED_HOSTILE_CASE_CODES
            ),
            "expected_positive_hostile_case_codes": list(
                alternate_evidence.POSITIVE_HOSTILE_CASE_CODES
            ),
            "expected_metric_a_opportunities": sum(
                int(summary["expected_metric_a_opportunities"])
                for summary in profile_alternate_summaries
            ),
            "event_count": len(profile_alternate_events),
            "false_opportunity_classifications": sum(
                int(summary["false_opportunity_classifications"])
                for summary in profile_alternate_summaries
            ),
            "episode_summaries": profile_alternate_summaries,
            "events": profile_alternate_events,
        }
...
def _validate_result(result: dict[str, object]) -> None:
    ...
    alternate = result["alternate_evidence"]
    if len(alternate_evidence.CASE_NAMES) != design.ALTERNATE_HOSTILE_CASES:
        raise ValueError("hostile alternate-evidence case count is not frozen")
    if int(alternate["guaranteed_valid_cases_per_episode"]) < 1:
        raise ValueError("alternate challenge has no guaranteed valid cases")
    if int(alternate["event_count"]) != len(alternate["events"]):
        raise ValueError("alternate event count mismatch")
    episode_count = len(result["epistemic_challenge"]["episodes"])
    expected_guaranteed = int(alternate["guaranteed_valid_cases_per_episode"]) * episode_count
    if int(alternate["guaranteed_opportunities"]) != expected_guaranteed:
        raise ValueError("alternate guaranteed denominator mismatch")
    if len(alternate["episode_summaries"]) != episode_count:
        raise ValueError("alternate episode summary count mismatch")
    expected_emitted_codes = list(alternate_evidence.EMITTED_HOSTILE_CASE_CODES)
    expected_represented_codes = list(
        alternate_evidence.HOSTILE_CASE_CODES
    )
    expected_positive_codes = list(
        alternate_evidence.POSITIVE_HOSTILE_CASE_CODES
    )
    expected_case_events = (
        int(alternate["guaranteed_valid_cases_per_episode"])
        + design.ALTERNATE_HOSTILE_CASES
        - 1
    )
    expected_case_opportunities = int(
        alternate["guaranteed_valid_cases_per_episode"]
    ) + len(expected_positive_codes) - 1
    expected_event_count = expected_case_events * episode_count
    if int(alternate["event_count"]) != expected_event_count:
        raise ValueError("alternate aggregate event count mismatch")
    if int(alternate["guaranteed_positive_event_count"]) != expected_guaranteed:
        raise ValueError("alternate guaranteed positive event count mismatch")
    if int(alternate["additional_hostile_event_count"]) != (
        len(expected_emitted_codes) * episode_count
    ):
        raise ValueError("alternate additional hostile event count mismatch")
    for summary in alternate["episode_summaries"]:
        if summary["event_count"] != expected_case_events:
            raise ValueError("alternate per-episode event count mismatch")
        if summary["represented_hostile_case_codes"] != expected_represented_codes:
            raise ValueError("alternate hostile case coverage mismatch")
        if summary["emitted_hostile_case_codes"] != expected_emitted_codes:
            raise ValueError("alternate emitted hostile case set mismatch")
        if summary["expected_positive_hostile_case_codes"] != expected_positive_codes:
            raise ValueError("alternate positive hostile case set mismatch")
        if summary["emitted_positive_hostile_case_codes"] != expected_positive_codes[1:]:
            raise ValueError("alternate emitted positive case set mismatch")
        if summary["additional_hostile_case_count"] != len(expected_emitted_codes):
            raise ValueError("alternate additional hostile case count mismatch")
        if summary["expected_metric_a_opportunities"] != expected_case_opportunities:
            raise ValueError("alternate expected opportunity count mismatch")
        episode_events = [
            event
            for event in alternate["events"]
            if event["episode_ordinal"] == summary["episode_ordinal"]
        ]
        if len(episode_events) != expected_case_events:
            raise ValueError("alternate event episode membership mismatch")
        if summary["guaranteed_positive_event_count"] != int(
            alternate["guaranteed_valid_cases_per_episode"]
        ):
            raise ValueError("alternate per-episode guaranteed count mismatch")
        if summary["false_opportunity_classifications"] != sum(
            bool(event["false_opportunity_classification"])
            for event in episode_events
        ):
            raise ValueError("alternate per-episode false classification mismatch")
        if summary["metric_a_opportunities"] != sum(
            bool(event["metric_a_opportunity"]) for event in episode_events
        ):
            raise ValueError("alternate per-episode Metric A mismatch")
        if summary["metric_a_successes"] != sum(
            bool(event["metric_a_success"]) for event in episode_events
        ):
            raise ValueError("alternate per-episode Metric A success mismatch")
    if alternate["represented_hostile_case_codes"] != expected_represented_codes:
        raise ValueError("alternate aggregate hostile coverage mismatch")
    if alternate["emitted_hostile_case_codes"] != expected_emitted_codes:
        raise ValueError("alternate aggregate emitted case mismatch")
    if alternate["expected_positive_hostile_case_codes"] != expected_positive_codes:
        raise ValueError("alternate aggregate positive case mismatch")
    if alternate["expected_metric_a_opportunities"] != expected_case_opportunities * episode_count:
        raise ValueError("alternate aggregate expected opportunity mismatch")
    if int(alternate["guaranteed_opportunities"]) < design.ALTERNATE_SUPPORT_MIN_OPPORTUNITIES:
        raise ValueError("alternate challenge denominator is below preregistered minimum")
    ...
    for event in alternate["events"]:
        if not required_event_fields.issubset(event):
            raise ValueError("alternate event is missing required evaluator fields")
    event_opportunities = sum(
        bool(event["metric_a_opportunity"]) for event in alternate["events"]
    )
    event_successes = sum(bool(event["metric_a_success"]) for event in alternate["events"])
    expected_event_opportunities = sum(
        bool(event["expected_metric_a_opportunity"])
        for event in alternate["events"]
    )
    event_false_classifications = sum(
        bool(event["false_opportunity_classification"])
        for event in alternate["events"]
    )
    if expected_event_opportunities != int(alternate["expected_metric_a_opportunities"]):
        raise ValueError("alternate evaluator expected opportunity mismatch")
    if event_false_classifications != int(
        alternate["false_opportunity_classifications"]
    ):
        raise ValueError("alternate false classification count mismatch")
    aggregate_a = result["epistemic_challenge"]["aggregate"][
        "alternate_support_preservation"
    ]
    if int(aggregate_a["opportunities"]) != event_opportunities:
        raise ValueError("Metric A aggregate opportunity mismatch")
    if int(aggregate_a["successes"]) != event_successes:
        raise ValueError("Metric A aggregate success mismatch")
    if int(aggregate_a["opportunities"]) < design.ALTERNATE_SUPPORT_MIN_OPPORTUNITIES:
        raise ValueError("Metric A denominator is below preregistered minimum")
    gates, verdict = _epistemic_gates(
        result["epistemic_challenge"]["aggregate"], alternate
    )
    if result["epistemic_gates"] != gates:
        raise ValueError("epistemic gate values do not match measured metrics")
    if result["epistemic_authority_verdict"] != verdict:
        raise ValueError("epistemic scientific verdict does not match measured gates")
    _finite(result)
...
def run_development_seed(model_seed: int) -> dict[str, object]:
    ...
    try:
        ...
        alternate_events: list[dict[str, object]] = []
        alternate_summaries: list[dict[str, object]] = []
        ordinal = 0
        for mode in MODES:
            for episode_seed in selection["challenge"][mode]:
                ...
                alternate_frames, episode_alternate_events, alternate_summary = _build_alternate_challenge(
                    ordinal,
                    start_tick=1_000_000 + ordinal * 100_000,
                    phase=phase,
                )
                challenge_frames.extend(alternate_frames)
                alternate_events.extend(episode_alternate_events)
        ...
        nursery_transition_metrics = aggregate_transitions(all_transitions)
        alternate_opportunities = sum(
            bool(event["metric_a_opportunity"]) for event in alternate_events
        )
        alternate_successes = sum(
            bool(event["metric_a_success"]) for event in alternate_events
        )
        transition_metrics = dict(nursery_transition_metrics)
        transition_metrics["alternate_support_preservation"] = {
            "opportunities": alternate_opportunities,
            "successes": alternate_successes,
            "rate": (
                alternate_successes / alternate_opportunities
                if alternate_opportunities
                else 1.0
            ),
        }
        transition_metrics["nursery_transition_diagnostics"] = (
            nursery_transition_metrics
        )
        flat_rows = benchmark_recompute_everything()
        scaling_flat = []
        for row in flat_rows:
        ...
        with _phase_context(
            ...
            output_sizes = {
                ...
                "alternate_event_count": len(alternate_events),
                "alternate_event_canonical_bytes": len(
                    _canonical_json_bytes(alternate_events)
                ),
            }
        runtime = {
        ...
        alternate_evidence_result = {
            "guaranteed_valid_cases_per_episode": design.ALTERNATE_VALID_CASES_PER_EPISODE,
            "guaranteed_opportunities": design.ALTERNATE_VALID_CASES_PER_EPISODE
            * len(alternate_summaries),
            "guaranteed_positive_event_count": design.ALTERNATE_VALID_CASES_PER_EPISODE
            * len(alternate_summaries),
            "additional_hostile_event_count": (
                len(alternate_evidence.EMITTED_HOSTILE_CASE_CODES)
            ...
            "event_count": len(alternate_events),
            "episode_summaries": alternate_summaries,
            "false_opportunity_classifications": sum(
                int(summary["false_opportunity_classifications"])
                for summary in alternate_summaries
            ),
        ...
        epistemic_gates, epistemic_verdict = _epistemic_gates(
            transition_metrics, alternate_evidence_result
        )
        ...
        result: dict[str, object] = {
            ...
            "alternate_evidence": alternate_evidence_result,
            "epistemic_gates": epistemic_gates,
            "epistemic_authority_verdict": epistemic_verdict,

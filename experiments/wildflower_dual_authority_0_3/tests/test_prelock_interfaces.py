...
from experiments.wildflower_dual_authority_0_3.design import (
    DEVELOPMENT_SEEDS,
    QUALIFICATION_SEEDS,
    selector_ranges,
    selector_starts,
)
from experiments.wildflower_dual_authority_0_3.qualification_guard import (
    assert_qualification_locked,
    assert_seed_is_registered,
    development_seed_is_allowed,
    qualification_is_locked,
)
from experiments.wildflower_dual_authority_0_3.predictive_trace import (
    PredictiveTrace,
    PredictiveTraceRow,
)
from experiments.wildflower_dual_authority_0_3.recorded_stream import (
...
from experiments.wildflower_dual_authority_0_3.scaling import (
...
def test_predictive_trace_is_finite_and_keeps_event_locations() -> None:
    ...
    trace.append(
        PredictiveTraceRow(
            ...
            gated_error=0.85,
            h8_prediction=(0.1, 0.3),
            event_location=8,
        )
    )
    assert trace.event_locations(340) == (8,)
...
def test_selector_ranges_are_disjoint_and_seed_guard_is_fail_closed() -> None:
    ...
    assert min(starts) >= 3_000_000
    assert selector_starts(340)["training"] == 3_000_000
    assert set(DEVELOPMENT_SEEDS).isdisjoint(QUALIFICATION_SEEDS)
    assert qualification_is_locked()
    assert development_seed_is_allowed(340)
    for spent_seed in (320, 321, 322, 330, 331):
        with pytest.raises(ValueError, match="0.2 seed"):
    ...
    with pytest.raises(RuntimeError, match="locked"):
        assert_qualification_locked(350)

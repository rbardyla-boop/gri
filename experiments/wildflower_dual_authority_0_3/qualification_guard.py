"""Fail-closed seed authorization for the unreleased 0.3 successor."""
...
def assert_seed_is_registered(seed: int) -> None:
    if seed in RESERVED_0_1_SEEDS:
        raise ValueError(f"0.1 seed is not reusable in successor: {seed}")
    if seed in RESERVED_0_2_SEEDS:
        raise ValueError(f"0.2 seed is not reusable in successor: {seed}")

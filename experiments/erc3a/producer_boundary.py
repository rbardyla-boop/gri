...
_FORBIDDEN_JSON_KEY = re.compile(
    r'"(?:sample_id|fault_target|sc_type|sc_location)"\s*:'
)
...
def assert_clean(paths: Iterable[Path]) -> None:
    ...
    if violations:
        raise AssertionError(f"producer-visible identity leakage: {json.dumps(violations, sort_keys=True)}")
...
def main() -> None:
    ...
    print("ERC3A_PRODUCER_IDENTITY_BOUNDARY_PASS")

...
def query_descriptor_from_write(row: dict[str, Any]) -> dict[str, Any]:
    ...
    for token in descriptor["tokens"]:
        text = str(token)
        if text.startswith("write_A_"):
            by_axis["A"] = text.replace("write_", "query_", 1)
        elif text.startswith("write_B_"):
            by_axis["B"] = text.replace("write_", "query_", 1)
    if set(by_axis) != {"A", "B"}:
        raise ValueError("write descriptor lacks one A and one B token")
    return {
        "attribute_order": ["B", "A"],
        "noise_token_count": 0,
        "tokens": [by_axis["B"], by_axis["A"]],
    }
